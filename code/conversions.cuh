...
template <typename T> 
__global__ void reduceMin(T* g_linedata, T* g_sumdata) {
	extern __shared__ float4 extdata[];
	T* sdata = (T*) extdata;
	...
	unsigned int tid = threadIdx.x;
	unsigned int i = blockIdx.x*blockDim.x*2 + threadIdx.x;
	sdata[tid] = fminf(g_linedata[i],g_linedata[i+blockDim.x]);
	__syncthreads();
...
template <typename T> 
__global__ void reduceMax(T* g_linedata, T* g_sumdata) {
	extern __shared__ float4 extdata[];
	T* sdata = (T*) extdata;
	...
	unsigned int tid = threadIdx.x;
	unsigned int i = blockIdx.x*blockDim.x*2 + threadIdx.x;
	sdata[tid] = fmaxf(g_linedata[i],g_linedata[i+blockDim.x]);
	__syncthreads();
